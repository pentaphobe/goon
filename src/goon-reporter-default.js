...
const verboseReport = (config, results) => {
  let report = results.fileDetails.reduce(
    (accum, fileDetail) => {
      return accum.concat(
        fileDetail.messages.map(message => {
          return [
            `${chalk.yellow(path.relative(__dirname, fileDetail.filePath))} ${
              message.line
            }:${message.column}`,
    ...
    [['file', 'message', 'debt'].map(title => chalk.cyan.bold(title))]
  )
...
const printDelta = delta =>
  delta > 0
    ...
    : delta === 0 ? chalk.bold.yellow(delta) : chalk.bold.green(delta)
...
---
TOTAL DEBT:  ${chalk.bold(total)} (${results.totals.errorCount} errors & ${
  results.totals.warningCount
} warnings)
